...
class Agip(Jurisdiccion):
    @classmethod
    async def create(cls, playwright: Playwright, cliente, cuit, clave_fiscal, fecha_desde, fecha_hasta, cuit_cliente_input):
        self = await super().create(playwright, "AGIP", "901 CABA", cliente, cuit, clave_fiscal, fecha_desde, fecha_hasta, cuit_cliente_input)
    ...
    async def consultar_notificaciones(self):
        ...
        await self.page.select_option("select[name='cuit_representado']", f"{self._cuit_cliente_input}")
        await self.page.click(f"xpath=//*[@onclick='ir_servicio(54,{self._cuit_cliente_input})']")
        await self.page.click("xpath=//*[@class='btnNoLeidas btn btn-default']", timeout=900000) # 15 min


    async def buscar_notificacion(self):
        hay_notificacion_no_leida = await super().buscar_notificacion(self.page, texto='---')
        self.hay_notificacion = not hay_notificacion_no_leida
        print(f"Notificaciones AGIP: {self.hay_notificacion}")
